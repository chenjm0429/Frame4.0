...
public class WheelView extends View {
     ...
     */
    private static final int ADDITIONAL_ITEM_HEIGHT = Level1Util.getDipSize(20);

    /**
     * Text size
     */
    private int TEXT_SIZE = Level1Util.getSpSize(18);
    ...
    private String buildText(boolean useCurrentValue) {
        ...
        for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
            if (useCurrentValue || i != currentItem) {
                ...
                if (text != null) {
                    ...
                    int tw = (int) itemsPaint.measureText(text);
                    if (maxTextWidth < tw) {
                        maxTextWidth = tw;
                        maxWidthText = text;
                    }
        ...
        if (maxTextWidth > itemsWidth) {//缩小文字
            while (itemsPaint.measureText(maxWidthText) > itemsWidth) {
                if (fontSize - 1 > 5) {
                    itemsPaint.setTextSize(fontSize - 1);
                    valuePaint.setTextSize(fontSize - 1);
                    fontSize = fontSize - 1;
                } else {
                    break;
                }
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
            float textWidth = FloatMath.ceil(Layout.getDesiredWidth("00", itemsPaint));
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        ...
        if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
            String showStr = buildText(isScrollingPerformed);
            itemsLayout = new StaticLayout(showStr, itemsPaint, widthItems,
                    widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE : Layout.Alignment
                            .ALIGN_CENTER,
                    1, ADDITIONAL_ITEM_HEIGHT + ratioHeight(TEXT_SIZE, itemsPaint.getTextSize()),
                    false);
        } else {
        ...
        if (widthLabel > 0) {
            if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
                labelLayout = new StaticLayout(label, valuePaint,
                        ...
                        ADDITIONAL_ITEM_HEIGHT + ratioHeight(TEXT_SIZE, itemsPaint.getTextSize())
                        , false);
            } else {
    ...
    private float ratioHeight(float textSize, float newSize) {
        ...
        float oldH = fm.bottom - fm.top;


        itemsPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
                | Paint.FAKE_BOLD_TEXT_FLAG);
        itemsPaint.setTextSize(newSize);
        fm = itemsPaint.getFontMetrics();
        float newH = fm.bottom - fm.top;
        return oldH - newH;
...
//		bottomShadow.draw(canvas);
    ...
    private void justify() {
        ...
        boolean needToIncrease = offset > 0 ? currentItem < adapter.getItemsCount() : currentItem
                > 0;
    ...
    public void setTEXT_SIZE(int tEXT_SIZE) {
        TEXT_SIZE = Level1Util.getSpSize(tEXT_SIZE);
    }
