...
public class DateTimeUI extends TextView {
    ...
    public void create(String selDate) {
        ...
        if (maxDate == null) {
            ...
            if (selectOrder[0] == 0) {
                selectOrder[1] = Integer.parseInt(selectValue[1]) - Integer.parseInt(minDate[1]);
            } else {
                selectOrder[1] = Integer.parseInt(selectValue[1]) - 1;
            }

            if (selectOrder[0] == 0 && selectOrder[1] == 0) {
                selectOrder[2] = Integer.parseInt(selectValue[2]) - Integer.parseInt(minDate[2]);
            } else {
    ...
    private String init(String selDate) {
        ...
        if (Long.parseLong(selDate) < Long.parseLong(minDate[0] + minDate[1] + minDate[2] +
                minDate[3] + minDate[4])) {
            selDate = minDate[0] + minDate[1] + minDate[2] + minDate[3] + minDate[4];
        }
        if (maxDate != null && Long.parseLong(selDate) > Long.parseLong(maxDate[0] + maxDate[1] +
                maxDate[2] + maxDate[3] + maxDate[4])) {
            selDate = maxDate[0] + maxDate[1] + maxDate[2] + maxDate[3] + maxDate[4];
        }
...
//            }
        ...
        mDialog.setCurrentDate(DateUtil.getInstance().getToday("yyyyMMdd"));
...
//            ad.show();
    ...
    private void getNamesAdapt(int[] tmp) {
        ...
        int minDay = 1, maxDay = DateUtil.getInstance().getDayOfCurrentMonth(minYear + tmp[0],
                minMonth + tmp[1]);
    ...
    /**
     * 设置默认日期，支持yyyy-MM-dd HH:mm
     *
     * @param selectDate
     */
    public void setShowValue(String selectDate) {
        selectDate = DateUtil.getInstance().convertDay_Type(selectDate, "yyyy-MM-dd HH:mm",
                "yyyyMMddHHmm");

        selectValue[0] = selectDate.substring(0, 4);
        selectValue[1] = selectDate.substring(4, 6);
        selectValue[2] = selectDate.substring(6, 8);
        selectValue[3] = selectDate.substring(8, 10);
        selectValue[4] = selectDate.substring(10);
        for (int i = 0; i < selectValue.length; i++) {
